...
def train(dataset, args):
    ...
    # Use only the set of in-distribution class
    ...
    x_train = x_train[df_train.label.isin(in_dist_class_set_labels)]
    df_train = df_train[df_train.label.isin(in_dist_class_set_labels)]
    x_val = x_val[df_val.label.isin(in_dist_class_set_labels)]
    df_val = df_val[df_val.label.isin(in_dist_class_set_labels)]

    # Mask out training data based on user specification
    if args.mask:
        x_train, df_train = _mask_data(x_train, df_train, args.mask)
        x_val, df_val = _mask_data(x_val, df_val, args.mask)
    ...
    x_train = utils.standardize(x_train, mean, std)
    x_val = utils.standardize(x_val, mean, std)
    pickle.dump((mean, std), open(os.path.join(model_path, 'scaler.p'), 'wb'))

    # Just for performance purposes - test data 
    from datasets import Datasets
    datasets = Datasets(args.dataset_path)
    ...
    training.train(x_train, y_train.values, df_train.index,
                   x_val, y_val.values, df_val.index,
                   log_path, model_path, x_test, y_test.values, df_test.index, **params)
